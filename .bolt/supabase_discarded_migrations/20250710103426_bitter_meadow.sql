/*
  # Update target_contact_lists to JSON Object Structure
  
  1. Changes
     - Change default value of target_contact_lists from '[]'::jsonb (array) to '{}'::jsonb (object)
     - Update triggers to handle target_contact_lists as a JSON object instead of array
  
  2. Affected Functions
     - expand_campaign_by_list: Updated to iterate over object keys (list IDs) instead of array elements
     - validate_target_contact_lists: Updated to validate keys in a JSON object
     - sync_campaign_contact_details: Updated to extract the first list ID from JSON object keys
     
  3. Trigger Conditions
     - Updated WHEN condition for trg_expand_campaign_by_list to check for object type and non-empty object
*/
...
BEGIN
    ...
    IF NEW.target_contact_lists IS NOT NULL AND jsonb_typeof(NEW.target_contact_lists) = 'object' AND jsonb_object_length(NEW.target_contact_lists) > 0 THEN
...
FOR EACH ROW
WHEN (NEW.target_contact_lists IS NOT NULL AND jsonb_typeof(NEW.target_contact_lists) = 'object' AND jsonb_object_length(NEW.target_contact_lists) > 0)
