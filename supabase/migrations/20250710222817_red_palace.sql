/*
  # Setup media-library storage bucket and fix JSONB campaign issues

  1. Storage Setup
    - Create media-library bucket for campaign media files
    - Enable RLS and create policies for authenticated users
    - Allow public read access for media files

  2. Campaign Target Lists Fix
    - Ensure target_contact_lists column is properly configured as JSONB
    - Fix any triggers that might be causing "key" column errors
    - Recreate validation and expansion triggers with proper JSON handling

  3. Security
    - Enable RLS on storage tables
    - Create appropriate policies for media access
*/
...
-- Set up RLS policies for the media-library bucket
...
CREATE POLICY "Authenticated users can update their media library files"
ON storage.objects
FOR UPDATE
TO authenticated
USING (bucket_id = 'media-library');

CREATE POLICY "Authenticated users can delete their media library files"
ON storage.objects
FOR DELETE
TO authenticated
USING (bucket_id = 'media-library');

-- Public access for viewing media library files
