/*
  ...
  2. Database Fixes
    - Ensure target_contact_lists column is properly configured
    - Fix any trigger issues that might be causing "key" column errors
*/
...
CREATE POLICY IF NOT EXISTS "Authenticated users can upload campaign media"
ON storage.objects
FOR INSERT
TO authenticated
WITH CHECK (bucket_id = 'campaign-media');

CREATE POLICY IF NOT EXISTS "Authenticated users can view campaign media"
ON storage.objects
FOR SELECT
TO authenticated
USING (bucket_id = 'campaign-media');

CREATE POLICY IF NOT EXISTS "Authenticated users can update their campaign media"
ON storage.objects
FOR UPDATE
TO authenticated
USING (bucket_id = 'campaign-media');

CREATE POLICY IF NOT EXISTS "Authenticated users can delete their campaign media"
ON storage.objects
FOR DELETE
TO authenticated
USING (bucket_id = 'campaign-media');
...
CREATE POLICY IF NOT EXISTS "Public can view campaign media"
ON storage.objects
FOR SELECT
TO public
USING (bucket_id = 'campaign-media');
