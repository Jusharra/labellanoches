/*
  # Campaign Expansion Function for JSONB Arrays

  1. Function Updates
    - `expand_campaign_by_list()` function to handle JSONB arrays
    - Properly check for null/empty JSONB arrays
    - Use `jsonb_array_elements_text()` to iterate over contact lists
    - Create individual campaign rows for each contact in the target lists

  2. Trigger Updates
    - Update trigger condition to work with JSONB arrays
    - Fire when `target_contact_lists` is not null and not empty

  3. Database Changes
    - Drop and recreate function with proper JSONB handling
    - Drop and recreate trigger with proper JSONB conditions
    - Add helpful comments for documentation
*/

-- Function to expand campaigns by contact list (JSONB version)
...
BEGIN
  ...
  SELECT
    ...
    jsonb_build_array(list_id_text::uuid), -- Preserve the source list ID for traceability as JSONB
