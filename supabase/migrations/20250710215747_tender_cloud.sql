/*
  # Update campaigns table to use JSON object for target_contact_lists

  1. Changes Made
    - Alter the default value of target_contact_lists to an empty JSON object
    - Update trigger function expand_campaign_by_list to handle JSON objects
    - Update trigger function validate_target_contact_lists to handle JSON objects  
    - Update trigger function sync_campaign_contact_details to handle JSON objects
    - Replace invalid jsonb_object_length function with proper PostgreSQL syntax

  2. Security
    - Maintain existing RLS policies
    - Preserve trigger functionality for campaign expansion
    - Keep validation and sync mechanisms intact

  3. Notes
    - Uses NEW.target_contact_lists <> '{}'::jsonb instead of jsonb_object_length
    - Handles both object and array formats for backward compatibility
    - Maintains all existing campaign processing logic
*/
...
BEGIN
    ...
    IF NEW.target_contact_lists IS NOT NULL AND jsonb_typeof(NEW.target_contact_lists) = 'object' AND NEW.target_contact_lists <> '{}'::jsonb THEN
...
FOR EACH ROW
WHEN (NEW.target_contact_lists IS NOT NULL AND jsonb_typeof(NEW.target_contact_lists) = 'object' AND NEW.target_contact_lists <> '{}'::jsonb)
