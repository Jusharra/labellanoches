/*
  # Campaign Expansion by Contact List

  1. Function
    - `expand_campaign_by_list()`: Expands campaigns by creating individual rows for each contact in target lists
    - Handles JSONB target_contact_lists properly
    - Only includes opted-in contacts
    - Generates unique airtable_record_id for each expanded campaign
    - Deletes original template row after expansion

  2. Trigger
    - `trg_expand_campaign_by_list`: Fires after campaign insertion when target_contact_lists is provided
    - Enables automatic campaign personalization for Whalesync integration
*/
...
BEGIN
  ...
  IF NEW.target_contact_lists IS NULL 
     OR NEW.target_contact_lists = 'null'::jsonb 
     OR jsonb_array_length(NEW.target_contact_lists) = 0 THEN
  ...
  SELECT
    ...
    jsonb_build_array(list_id), -- Preserve the source list ID for traceability as JSONB
    NEW."Personalized SMS Text", -- Copy personalized SMS text
    NEW."Personalized WhatsApp Text" -- Copy personalized WhatsApp text
  FROM jsonb_array_elements_text(NEW.target_contact_lists) AS list_id
...
COMMENT ON FUNCTION expand_campaign_by_list() IS 'Automatically expands campaigns by creating individual rows for each contact in the target contact lists (JSONB). This enables Whalesync to sync personalized campaigns to Airtable.';
